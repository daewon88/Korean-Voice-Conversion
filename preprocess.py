...
def coded_sps_normalization_fit_transoform(coded_sps):
    ...
    for i,coded_sp in enumerate(coded_sps):
        if i<3:
            print('mean : ', coded_sps_mean)
            print('coded_sp shape : ', coded_sp)
            print('normalized : ', ((coded_sp - coded_sps_mean) / coded_sps_std) )
...
'''
file_path = 'mv05/mv05_t01_s01.wav'
sr = 16000
wav, _ = librosa.load(file_path, sr = sr, mono = True)
wavs = [wav]
f0s, timeaxes, sps, aps, coded_sp = world_encode_data(wavs,sr)
print(len(f0s[0]))
print(len(timeaxes[0]))
print(np.array(sps).shape)
print(np.array(aps).shape)
print(np.array(coded_sp).shape)
'''
